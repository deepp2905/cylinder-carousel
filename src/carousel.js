...
// --- Settings panel toggle (Media uploader only — DialKit handles the rest) ---
const panel = document.getElementById('settings-panel');
document.getElementById('settings-toggle').onclick = () => panel.classList.toggle('open');

// --- Setters wired to DialKit (see controls.jsx) ---
export const setters = {
    cameraZ(val) { config.cameraZ = val; camera.position.z = val; },
    height(val) {
        config.height = val;
    ...
    autoDur(val) { config.autoDur = val; if (!isMoving) startAutoplay(); },
    arrowPad(val) { document.documentElement.style.setProperty('--arrow-pad', val + 'px'); },
    pillBottom(val) { document.documentElement.style.setProperty('--pill-bottom', val + 'px'); },
    audioHigh(val) { config.audioHigh = val; },
    audioLow(val) { config.audioLow = val; },
    audioDur(ms) { config.audioDur = ms / 1000; }
};
...
export const defaults = {
    ...
    autoDur: config.autoDur,
    arrowPad: 40,
    pillBottom: 40,
    audioHigh: config.audioHigh,
    audioLow: config.audioLow,
    audioDur: config.audioDur * 1000
};
